...
@section('content')
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('attendance.store') }}" method="POST" class="mt-n3">
                                @csrf
                                ...
                                @if (count($employees) > 0)
                                    <table id="attendance_table" class="table table-hover nowrap">
                                    ...
                                    <div id="next-to-table" class="d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center pl-3">
                                            <label class="custom-checkbox m-0 pb-1"><input type="checkbox"
                                                    class="check-all-box"><span class="custom-label"></span></label>
                                            <p class="m-0">Check All</p>
                                        </div>
                                        <div class="d-flex align-items-center pr-3">
                                            <button id="btn-cancel" class="btn btn-outline-danger mr-2"
    ...
    @include('inc.confirmDeletion', ['title' => 'attendance'])
@endsection
...
@push('script')
    <script>
        ...
        function checkAttendance() {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('attendance.check', [':date', ':shift']) }}".replace(':date', $('#date').val())
                    .replace(':shift', $('#shift').val()),
                dataType: 'json',
        ...
        function displayEmployee(data) {
            ...
            data.forEach((employee, index) => {
                ...
                let present = $('<td class="align-middle"></td>').html(
                    '<label class="custom-checkbox"><input type="checkbox" name="attendance[' + employee
                    .employee_id + '][present]" ' + (employee.present == 1 ? "checked" : "") + ' class="' + (
                        employee.in_vacation ? "" : "check-box") + '" ' + (employee.in_vacation ? "disabled" :
                        "") + '><span class="custom-label"></span></label>'
                );
                let name = $('<td class="align-middle"></td>').html('<a href="' + url + '">' + employee.full_name +
                    '</a><input type="hidden" name="attendance[' + employee.employee_id +
                    '][employee_id]" value="' + employee.employee_id + '">');
                let time_in = $('<td class="align-middle"></td>').html(
                    '<input type="time" name="attendance[' + employee.employee_id + '][time_in]" min="' +
                    starting_time + '" max="' + leaving_time +
                    '" value="' + employee.time_in + '" ' + (employee.present == 0 || employee.in_vacation ?
                        "readonly" : "") + '  class="form-control form-control-sm ' + (employee.in_vacation ?
                        "" : "time-in") + '">');
                let time_out = $('<td class="align-middle"></td>').html(
                    '<input type="time" name="attendance[' + employee.employee_id + '][time_out]" min="' +
                    starting_time + '" max="' + leaving_time +
                    '" value="' + employee.time_out + '" ' + (employee.present == 0 || employee.in_vacation ?
                        "readonly" : "") + ' class="form-control form-control-sm ' + (employee.in_vacation ?
                        "" : "time-out") + '">');
                let note = $('<td class="align-middle"></td>').html(
                    '<textarea name="attendance[' + employee.employee_id + '][note]" ' + (employee.present ==
                        0 || employee.in_vacation ? "readonly" : "") +
                    ' cols="40" rows="2" class="form-control ' + (employee.in_vacation ? "" : "note") + '">' +
                ...
                let badges = $('<td class="align-middle"></td>').html(
                    (employee.in_vacation ? '<span class="badge badge-info mr-1">in-vacation</span>' :
                        (!employee.has_attendance ?
                            '<span class="badge badge-danger mr-1">no-attendance</span>' :
        ...
        $('body').on('change', '.check-box', function() {
            ...
            if (this.checked) {
                ...
                time_in_td.val(starting_time);
                time_out_td.val(leaving_time);
                $('.check-all-box').prop('checked', $('#attendance_table .check-box:checked').length === $(
                    '#attendance_table .check-box').length);
            } else {
                time_in_td.prop('readonly', true);
                time_out_td.prop('readonly', true);
                note_td.prop('readonly', true);
