...
class Employee extends Model
{
    ...
    public function getVacationDays() {
        ...
        $totalVacations = $months * $setting->vacation_rate;
        return ($totalVacations - $takenVacation);
    }
    public function getTakingVacationStartAt() {
        $setting = Setting::first();
        return date('Y-m-d', strtotime("+$setting->taking_vacation_allowed_after months", strtotime($this->vacation_start_count_at)));
    }
    ...
    public function getLeewayDiscount($month) {
        $setting = Setting::first();
        return number_format(($this->getTotalLeeway($month) / 60) * $setting->leeway_discount_rate * $this->getHourlyPrice(), 2);
    }
